...
*** Test Cases ***
TC_005_update_existing_resource
    Create Session         Add_Data       ${Base_URL}
    &{body}=              Create Dictionary   first_name=Afzhal     middle_name=Don   last_name=S     date_of_birth=12/12/2002
    &{header}=            create dictionary  Content-Type=application/json
    ${response}=           put on session   Add_Data     /api/studentDetails/1515     data=${body}     headers=${header}
